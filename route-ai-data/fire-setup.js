...
const insightsData = []; // If you have any, add them here
...
// ... (copy the full routes, schedules, userPositions, waitingUsers from data.ts here) ...

// For brevity, I'll show the structure for one, but you must copy the full arrays from your data.ts:
const routes = [
    {
        id: "12",
        name: "Route 12",
...
async function uploadCollection(collectionPath, dataArray, wrapJson = false) {
    for (const item of dataArray) {
        const dataToUpload = wrapJson ? { json: JSON.stringify(item) } : item;
        const ref = item.id
            ? doc(db, collectionPath, String(item.id))
            : doc(collection(db, collectionPath));
        await setDoc(ref, dataToUpload);
    }
    console.log(`Uploaded ${dataArray.length} documents to ${collectionPath}`);
}

async function uploadDocument(documentPath, dataObject) {
    await setDoc(doc(db, documentPath), dataObject);
    console.log(`Uploaded document to ${documentPath}`);
}

async function main() {
    // // a) buses
    // await uploadCollection('town-milton/buses/bus-collection', buses);

    // // b) feedback
    // await uploadCollection('town-milton/feedback/feedback-collection', feedbackData);

    // // c) insights
    // await uploadCollection('town-milton/feedback/feedback-collection', insightsData);

    // // d) route colors (document)
    // await uploadDocument('town-milton/route-colors', routeColors);

    // // e) routes
    // await uploadCollection('town-milton/routes/route-collection', routes, true);

    // Add id property to each schedule using its routeId
    schedules.forEach(schedule => {
        schedule.id = schedule.routeId;
    });

    // f) schedules
    await uploadCollection('town-milton/schedules/schedule-collection', schedules, true);

    // // g) user positions
    // await uploadCollection('town-milton/user-positions/user-positions-collection', userPositions);

    // // h) waiting users
    // await uploadCollection('town-milton/waiting-users/waiting-users-collection', waitingUsers);
